...
class AggCluImpute:
    def __init__(self, coor_dire, nimp_dire, ann_dire, h="pos"):
        ...
        self.nimp_data = pd.read_csv(nimp_dire, sep="\t").rename(rename_dict, axis=1)
        self.data = pd.read_csv(coor_dire, sep="\t").rename(rename_dict, axis=1)
        self.ann = pd.read_csv(ann_dire, sep="\t").rename(rename_dict, axis=1)
        self.preprocess()
    
    def preprocess(self):
        assert np.ptp(self.ann[self.h]) == np.ptp(self.data[self.h])
        assert np.ptp(self.ann[self.h]) == np.ptp(self.nimp_data[self.h])
        self.miss_sr = self.nimp_data.groupby(self.c).apply(
            lambda x: np.ptp(self.ann[self.h]) + 1 - x.shape[0]
        )
        self.data[self.h] = self.data[self.h] + \
            (min(self.ann[self.h]) - min(self.data[self.h]))
        self.nimp_data[self.h] = self.nimp_data[self.h] + \
            (min(self.ann[self.h]) - min(self.nimp_data[self.h]))
        ...
        self.data_range = (self.data[self.coor] - self.data[self.coor].min()).mean(axis=1).median()
        self.data[self.coor] = self.data[self.coor]/self.data_range

        self.nimp_data[self.coor] = self.nimp_data[self.coor] - self.nimp_data[self.coor].min()
        nimp_data_range = (self.nimp_data[self.coor] - self.nimp_data[self.coor].min()).mean(axis=1).median()
